...
const AllWordResults = ({ query, page }) => {
  ...
  const getData = async () => {
    const res = await fetch(
      `https://contextualwebsearch-websearch-v1.p.rapidapi.com/api/Search/WebSearchAPI?q=${value}&pageNumber=${page}&pageSize=10&autoCorrect=true`,
      {
        method: "GET",
        headers: {
      ...
      <main className="w-full col-span-3 flex flex-col items-center">
        ...
        <ul className="w-full flex mt-16 border-b">
          <Link href={`/search/${query}/${page}`}>
            <li className="py-3 px-4 border-t border-r font-semibold cursor-pointer">
              All
            </li>
          </Link>
        ...
        {data.value &&
          data.value.map((item, index) => (
            <div key={index} className="w-full p-6 border-b">
        ...
        {data.value && (
          <ul className="flex justify-center gap-5 p-16 w-full">
            {parseInt(page) !== 1 && (
              <Link href={`/search/${query}/${parseInt(page) - 1}`}>
